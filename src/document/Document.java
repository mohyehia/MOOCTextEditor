...
public abstract class Document {
	...
	protected int countSyllables(String word) {
		...
		for (int i = 0; i < len; i++) {
			// check if e is the last character of the word
			if (len != 1 && i == (len - 1) && arr[i] == 'e' && vowels.indexOf(arr[i - 1]) != -1 && numOfSyllables > 0)
				continue;
			// check if char is vowel & previous char is not vowel
			if (i > 0 && i != (len - 1) && vowels.indexOf(arr[i]) != -1 && vowels.indexOf(arr[i - 1]) == -1)
				numOfSyllables++;
			if (i == 0 && vowels.indexOf(arr[i]) != -1)
				numOfSyllables++;
		}
	...
	public double getFleschScore() {
		...
		double wordsPerSentences = getNumWords() / getNumSentences();
		double syllablesPerWords = getNumSyllables() / getNumWords();
		double fleshScore = 206.835 - (1.015 * wordsPerSentences) - (84.6 * syllablesPerWords);
		return fleshScore;
	}
}
